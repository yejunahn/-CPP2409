...
int main() {
    ...
    while (true) {
        ...
        // 명령 입력
        string user_input;
        cout << "명령어를 입력하세요 (상, 하, 좌, 우, 지도, 종료, 정보): ";
        cin >> user_input;
        ...
        if (user_input == "상") {
            ...
            cout << player << endl;  // 사용자 정보 출력
            map[player.getY()][player.getX()] = 5;  // 현재 위치 표시
            continue;
        } else {
...
void interactWithPosition(vector<vector<int>>& map, User& user) {
    switch (map[user.getY()][user.getX()]) {
        case 1:
            ...
            user.eatItem();  // 아이템을 먹었을 때 itemCnt 증가
            map[user.getY()][user.getX()] = 0; // 아이템은 일회성이므로 발견 후 빈 공간으로 바꿈
            break;
        case 2:
